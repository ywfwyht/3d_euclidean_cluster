...
ros::Publisher pub_detected_bbox;
...
void publishDetectedObjects(const autoware_msgs::CloudClusterArray &in_clusters)
{
  ...
  jsk_recognition_msgs::BoundingBoxArray bbox_detected_array;
  bbox_detected_array.header = in_clusters.header;
  ...
  for (size_t i = 0; i < in_clusters.clusters.size(); i++)
  {
    ...
    bbox_detected_array.boxes.push_back(in_clusters.clusters[i].bounding_box);
  }
  _pub_detected_objects.publish(detected_objects);
  pub_detected_bbox.publish(bbox_detected_array);
}
...
{
  // std::cerr <<"frame:  " << in_target_frame << std::endl;
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  std::cerr << in_sensor_cloud->header << std::endl;
  ROS_INFO("frame_id: %s",in_sensor_cloud->header.frame_id.c_str());
...
int main(int argc, char **argv)
{
  ...
  pub_detected_bbox = h.advertise<jsk_recognition_msgs::BoundingBoxArray>("/bbox_detected", 10);
  ...
  else
  {
    ...
    points_topic = "/percept_cloud_rviz";
  }
